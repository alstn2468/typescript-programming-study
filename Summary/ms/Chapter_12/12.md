...
-   12-4 리액트와 부트스트랩으로 프론트엔드 웹 만들기 [:link:](#12-4-리액트와-부트스트랩으로-프론트엔드-웹-만들기)
    -   실습 프로젝트 구성 [:link:](#project-setup-three)
    -   App.tsx 파일 수정 [:link:](#apptsx-파일-수정)
    -   JSX란? [:link:](#jsx란)
    -   컴포넌트란? [:link:](#컴포넌트란)
    -   API 서버에서 실제 데이터 가져오기 [:link:](#api-서버에서-실제-데이터-가져오기)
    -   useState 함수 사용하기 [:link:](#usestate-함수-사용하기)
    -   useEffect 함수 사용하기 [:link:](#useeffect-함수-사용하기)
    -   서버에서 데이터 계속 가져오기 [:link:](#서버에서-데이터-계속-가져오기)
    -   부트스트램 CSS 프레임워크 사용하기 [:link:](#부트스트램-css-프레임워크-사용하기)
    -   카드 컴포넌트 만들기 [:link:](#카드-컴포넌트-만들기)
...
위와 같이 `res.json(...)` 부분이 실행되어 `JSON` 포맷 데이터가 출력되는 것을 볼 수 있다.<br/>
...
위와 같이 접속에 사용된 주소의 `skip`과 `limit`값이 출력되는 것을 확인할 수 있다.<br/>
...
서버를 재가동한 후 `localhost:4000/users/1/2`의 경로로 접속하면 아래와 같은 결과를 얻을 수 있다.<br/>
...
`App.tsx` 파일을 위와 같이 수정한 후 저장하면 아래와 같이 변경된다.<br/>
...
위와 같이 수정하면서 `getDataPromise` 함수는 `App` 컴포넌트가 처음 **렌더링**될 때만 실행된다.<br/>
...
위와 같이 `more data...` 버튼을 클릭할 경우 서버에서 데이터를 추가적으로 가져오는 것을 볼 수 있다.<br/>
...
`CSS` 부분은 `head` 태그 내부에 추가하고 `script` 부분은 `body` 내부에 추가한다.<br/>
